...
const Facets = () => {
    ...
    // const { secondLevelFacets } = useSelector((state: DefaultRootStateProps) => state.healthContent);

    const secondLevelFacets = {
        metricsGood: [
            {
                productId: 1,
                completeness: 84,
    ...
    useEffect(() => {
        setIsLoading(true);
        dispatch(getSecondLevelFacets());
    }, [dispatch]);

    useEffect(() => {
        setSecondLevel(secondLevelFacets);
        setIsLoading(false);
    }, [secondLevelFacets]);
    useEffect(() => {
        setIsLoading(true);
